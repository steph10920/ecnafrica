...
export default function Navbar() {
  ...
  return (
    <header className={`fixed w-full z-50 bg-white/95 backdrop-blur-md shadow-md transition-all ${isScrolled ? "py-1" : "py-2"}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => handleNavClick("/")} className="flex items-center gap-2">
          <img src={Logo} alt="ECN Logo" className={`transition-all rounded-full ${isScrolled ? "h-12" : "h-16"}`} />
          <span className="text-green-700 font-bold text-xl md:text-2xl tracking-wide">ECN</span>
        </button>
        ...
        <nav className="hidden md:flex items-center space-x-6 text-gray-800 font-medium">
          ...
          <div className="relative">
            <button className="hover:text-green-600 flex items-center gap-1" onClick={() => setCategoriesOpen(!categoriesOpen)}>
              Categories ▾
            </button>
            <AnimatePresence>
              {categoriesOpen && (
                ...
                >
                  {categories.map(([path, label]) => (
                    <button key={path} onClick={() => handleNavClick(`/categories/${path}`)} className="block w-full text-left px-4 py-2 hover:bg-green-50 hover:text-green-700">
                      {label}
                    </button>
                  ))}
                </motion.div>
          ...
          <form onSubmit={handleSearchSubmit}>
            <input
              ...
              placeholder="Search..."
              className="px-3 py-1 rounded-md border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          </form>
        </nav>

        {/* Mobile Toggle */}
        <button className="md:hidden text-green-700 text-3xl focus:outline-none" onClick={() => setMobileOpen(!mobileOpen)}>
          {mobileOpen ? "✕" : "☰"}
        </button>
      </div>
          ...
          >
            <div className="flex flex-col px-4 py-4 space-y-3">
              ...
              <details className="group">
                <summary className="cursor-pointer hover:text-green-600 py-2 border-b border-gray-200">Categories ▾</summary>
                <div className="pl-4 mt-2 flex flex-col space-y-1">
                  {categories.map(([path, label]) => (
                    <button key={path} onClick={() => handleNavClick(`/categories/${path}`)} className="hover:text-green-600 text-left">
                      {label}
                    </button>
                  ))}
                </div>
